...
impl Backend {
    ...
    async fn scan_workspace(&self) {
        ...
        let handle = tokio::task::spawn_blocking(move || {
            ...
            asset_index::scan(&analyzer, &roots, &base_roots, &mut progress)
        });
        ...
        let base_ini_count = result
            .entries
            .iter()
            .filter(|entry| entry.is_base && entry.models.is_empty())
            .count();
        self.base_indexed_count
            .store(base_ini_count, Ordering::Relaxed);
        self.last_scan_cache_hits
            .store(result.stats.hits, Ordering::Relaxed);
        self.scan_finished.store(true, Ordering::Relaxed);
        let ini_total = result
            .entries
            .iter()
            .filter(|entry| entry.models.is_empty())
            .count();
        let model_total: usize = result.entries.iter().map(|entry| entry.models.len()).sum();
        ...
        let apply_started = Instant::now();
        {
            ...
            for entry in result.entries {
                ...
                if !open.contains(&entry.file) {
                    ...
                    idx.set_file_tags(&entry.file, entry.tags);
                    idx.set_file_models(&entry.file, entry.models);
                }
            }
        }
        ...
        self.client
            .log_message(
                ...
                format!(
                    ...
                    result.stats.discovery.as_millis(),
                    result.stats.cache_load.as_millis(),
                    result.stats.parse.as_millis(),
                    result.stats.cache_write.as_millis(),
                    apply.as_millis(),
                    result.stats.hits,
                    result.stats.misses,
                ),
...
impl LanguageServer for Backend {
    ...
    async fn execute_command(
        ...
        let base_roots = self
            ...
            .map(|roots| roots.clone())
            .unwrap_or_default();
        let cleared = match asset_index::clear_cache(&roots, &base_roots) {
            Ok(cleared) => cleared,
            Err(error) => {
...
mod tests {
    ...
    fn canonical_uri_pass_through_non_file() {
        let uri = Url::parse("untitled:///buffer").unwrap();
        assert_eq!(canonical_uri(uri.clone()), uri);
    }
